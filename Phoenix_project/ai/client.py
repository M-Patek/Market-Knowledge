...
class AIClient(Protocol):
    ...
    async def get_batch_asset_analysis(self, tickers: List[str], date_obj: datetime.date) -> Dict[str, Dict[str, Any]]:
        ...
...
class GeminiAIClient:
    ...
    async def get_market_sentiment(self, headlines: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            self._save_audit_record("market_sentiment", prompt, raw_text, context={"headlines": headlines})
            ...
            return {"sentiment_score": 0.0, "reasoning": "Fallback due to validation error."}
        except Exception as e:
    ...
    async def get_batch_asset_analysis(self, tickers: List[str], date_obj: datetime.date) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            response = await self._generate_with_timeout(prompt, generation_config=self.generation_config)
            raw_text = response.text
            self._save_audit_record("batch_asset_analysis", prompt, raw_text, context=context)
            ...
            return {ticker: model.model_dump() for ticker, model in validated_data.items()}
        except (json.JSONDecodeError, ValidationError) as e:
            ...
            return {ticker: fallback_analysis for ticker in tickers}
        except Exception as e:
    ...
    async def generate_summary_report(self, metrics: Dict[str, Any]) -> str:
        ...
        self._save_audit_record("summary_report", prompt, response.text, context=context)
...
class MockAIClient:
    ...
    async def get_market_sentiment(self, headlines: List[str]) -> Dict[str, Any]:
        ...
        return {"sentiment_score": 0.1, "reasoning": "Mocked neutral sentiment."}
    ...
    async def generate_summary_report(self, metrics: Dict[str, Any]) -> str:
        ...
        await asyncio.sleep(0.01)
        return "## Mock Marshal's Report\n\nThis is a simulated report based on mock data. All systems performed nominally."
