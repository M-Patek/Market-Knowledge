...
class SupplyChainIntelligenceAgent(L1Agent):
    ...
    def __init__(
        ...
        """
        初始化 SupplyChainIntelligenceAgent。
        ...
        logger.info(f"[{self.agent_id}] SupplyChainIntelligenceAgent initialized.")
    ...
    async def run(self, task: Task, context: List[Any]) -> AsyncGenerator[EvidenceItem, None]:
        ...
        """
        logger.info(f"[{self.agent_id}] Running supply chain analysis for task: {task.task_id} on symbols: {task.symbols}")
        ...
        agent_prompt_name = "l1_supply_chain_intelligence" 
