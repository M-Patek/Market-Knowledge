...
class DataManager:
    def __init__(self, config: StrategyConfig, cache_dir: str = "data_cache", snapshot_id: Optional[str] = None):
        ...
        # [NEW] Provider dispatch table for cleaner fetch logic
        self._provider_fetch_methods = {
    ...
    async def _fetch_and_cache_ticker_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        for provider_name in [p for p in self.ds_config.priority if self._is_provider_healthy(p)]:
            start_time = time.time()
            PROVIDER_REQUESTS_TOTAL.labels(provider=provider_name).inc()
            self.logger.info(f"Attempting to fetch '{ticker}' with provider: {provider_name.upper()}")
            try:
                fetch_method = self._provider_fetch_methods.get(provider_name)
                if not fetch_method:
                    self.logger.warning(f"No fetch method defined for provider '{provider_name}'. Skipping.")
                    continue
                data = await fetch_method(**fetch_params)

                if data is not None and not data.empty:
                    PROVIDER_LATENCY_SECONDS.labels(provider=provider_name).observe(time.time() - start_time)
                    self.logger.info(f"Successfully fetched data for '{ticker}' from {provider_name.upper()}.")
                    ...
                    if not data.empty:
                        ...
                        PROVIDER_DATA_FRESHNESS_SECONDS.labels(provider=provider_name).set(freshness_seconds)
                    ...
                    await self._write_cache_direct(cache_path, combined_df, provider_name, params)
                    return combined_df
                else:
                    raise DataFetchError(f"Provider {provider_name.upper()} returned no data for {ticker}.")
            except Exception as e:
                self.logger.error(f"Provider {provider_name.upper()} failed for '{ticker}': {e}")
                PROVIDER_ERRORS_TOTAL.labels(provider=provider_name).inc()
                PROVIDER_LATENCY_SECONDS.labels(provider=provider_name).observe(time.time() - start_time)
