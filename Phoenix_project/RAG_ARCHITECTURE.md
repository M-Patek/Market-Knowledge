...
Version: 2.4 (已根据代码库 v2.1+ 和 ai/graph_db_client.py 的实现更新)
...
非结构化文本 (News, Filings): models/text-embedding-004 (Google)。[根据 config/system.yaml 和 ai/embedding_client.py 确认]。
...
[审查确认 (2025-11-13)]: 本文档的描述与 requirements.txt, config/system.yaml (temporal_db) 和 ai/temporal_db_client.py (AsyncElasticsearch 实现) 完全一致。
...
技术 (Technology): PostgreSQL (psycopg2-binary) (如 requirements.txt 和 config/system.yaml 所示)。
...
[已确认] 高级检索已实现: ai/retriever.py 代码中完全实现了高级检索功能。
...
KnowledgeGraphService (knowledge_graph_service.py) 协调这些模块，以更新图谱。
关键更新： 与旧版文档不同，系统不再使用存根 (stub)。 ai/graph_db_client.py 中已实现了一个完整的、异步的 Neo4j 客户端 (AsyncGraphDBClient)，它直接连接到 phoenix_neo4j 容器，并执行实际的 Cypher 查询以进行 upsert_relations (写入) 和 query_graph (读取)。

requirements.txt 中包含 tensorflow-gnn，表明 GNN (图神经网络) 训练是此架构的预期部分 (如 training/gnn/gnn_engine.py 所示)，但 GNN 推理模型尚未集成到实时的 ai/graph_encoder.py 或 ai/retriever.py 中。
