...
class GeminiService:
    ...
    async def get_asset_analysis(self, ticker: str, date_obj: datetime.date) -> Dict[str, Any]:
        ...
        mock_response = {"adjustment_factor": factor, "reasoning": reasoning}
        self.logger.info(f"Received mock analysis for {ticker}: factor={mock_response['adjustment_factor']:.2f}")
        ...
        #     response = await self._generate_with_timeout(prompt, generation_config=self.generation_config)
        #     response_data = json.loads(response.text)
        #     if 'adjustment_factor' not in response_data or 'reasoning' not in response_data:
        #         raise MalformedResponseError(f"Missing required keys in asset analysis response: {response_data}")
        #     self.logger.info(f"Received valid asset analysis for {ticker}: factor={response_data['adjustment_factor']:.2f}")
        #     return response_data
        # except Exception as e:
